    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-4 py-4 border-b border-gray-800">
          ...
          <button onClick={() => setOpen(false)} className="text-gray-400 hover:text-red-400 transition">
            ✕
          </button>
        </div>
      ...
      👤 Add your name
        ...
        <nav className="px-4 py-4 space-y-1 overflow-y-auto h-[calc(100vh-160px)]">
          <div className="space-y-3 mb-6">
            <Link onClick={() => setOpen(false)} href="/" className="block text-gray-200 hover:text-indigo-300">
              📊 Dashboard
            </Link>

            <Link onClick={() => setOpen(false)} href="/history" className="block text-gray-200 hover:text-indigo-300">
              📜 History
            </Link>

            <Link onClick={() => setOpen(false)} href="/categories" className="block text-gray-200 hover:text-indigo-300">
              🏷️ Categories
            </Link>
          </div>
          ...
          <div className="mt-8 pt-4 border-t border-gray-800 text-[11px] text-gray-400 flex flex-col gap-3">
            <Link onClick={() => setOpen(false)} href="/legal/privacy">
              Privacy Policy
            </Link>
            <Link onClick={() => setOpen(false)} href="/legal/terms">
              Terms of Service
            </Link>
            <Link onClick={() => setOpen(false)} href="/legal/cookies">
              Cookie Policy
            </Link>
